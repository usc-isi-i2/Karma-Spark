...
if __name__ == '__main__':
    ...
    input_rdd = sc.textFile(input_path).map(lambda x: json.loads(x)).map(lambda x: (x['cdr_id'], x)).mapValues(remove_ist_extractions).mapValues(json.dumps).partitionBy(20)
